...
const objectToHTML = (song) => {
  ...
  <div class='col-1'>
    <img src="${song.image}" class="rounded" style='width: 55px; height: 55px;'>
  </div>
  <div class='col-11'>
...
const search = () => {
  ...
  let newSongL= [];
  //sq = search query
  let sq = document.querySelector('.js-input').value.toLowerCase();
  
  for(let i = 0; i < playlist.songs.length; i++) {
    if(playlist.songs[i].name.toLowerCase().includes(sq)){
      newSongL.push(playlist.songs[i]);
      //console.log(newSongL)
    }
    //let sq = document.querySelector('.js-input').value.toLowerCase();
    //console.log(document.querySelector(".js-input").value.toLowerCase());
    // if(playlist.songs.tolowercase.includes(sq)){
    //   newSongL.push(playlist.songs[i])
    // }
  }
  newPlaylist.songs = newSongL; 
  render(newPlaylist);  
//console.log(newSongL);
}
...
//ADD EVENTLISTER SOMEWHERE IN SEARCH
